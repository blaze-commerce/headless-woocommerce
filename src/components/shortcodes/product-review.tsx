...
import { Camera } from '@src/components/svg/camera';
...
export function ProductReview({ attributes }: { attributes: ShortcodeAttribute[] }) {
  const defaultAttributes: { [key: string]: string } = {
    card_style: 'shadow-card',
  ...
  const renderJudgeMeReviewsCount = () => {
    ...
    return (
      <>
        <div className="flex justify-center mt-5">
          {(starReviews as number) > 0 ? pageItemsCount : currentItemsCount} out of{' '}
          {(starReviews as number) > 0 ? totalPageItems : totalItems}
        </div>
        {(isRatingPageLessThanTotal || isPageLessThanTotal) && renderLoadMoreButton()}
      </>
    );
  ...
  const renderReviewItems = () => {
    ...
    const filterWooProductReviews = reduce(
      ...
      (result: WoocommerceProductReviews[], review) => {
        ...
        if (selectedReviewsWithImage && (!isArray(review.images) || review.images.length === 0))
          pushReview = false;

        if (selectedVerifiedReviews && !review.verified) pushReview = false;

        if (pushReview) result.push(review as WoocommerceProductReviews);
      ...
      <>
        <div className="w-full">
            ...
            product?.judgemeReviews?.content?.slice(0, 5).map((data, index) => {
              return (
                <ReviewItem
                  key={`judgeme-${index}`}
                  {...data}
                  card_style={defaultAttributes.card_style}
                  hide_footer={defaultAttributes.hide_footer === 'true'}
                />
              );
            })}
          {reviewService === 'yotpo' &&
          ...
          {reviewService === 'woocommerce_native_reviews' && !starReviews && (
            <div
              className={`flex flex-col lg:grid grid-cols-${defaultAttributes.total_card} my-2.5 gap-10`}
            >
              {filterWooProductReviews.map((data: WoocommerceProductReviews, index: number) => {
                return (
                  <ReviewItem
                    key={`woocommerce-comment-${data.comment_ID}-${index}`}
                    {...data}
                    card_style={defaultAttributes.card_style}
                    hide_footer={defaultAttributes.hide_footer === 'true'}
                  />
  ...
  const renderReviewsInfo = () => {
    return (
      <div
        className={cn('block space-y-10 md:flex justify-between items-start h-full w-full', {
      ...
      >
        ...
        {reviewService === 'judge.me' && (
          <button
            ...
            className="flex items-center justify-center border border-brand-primary bg-white py-4 px-10 rounded-md w-48 h-10 text-brand-primary mx-auto md:mx-0"
          >
  ...
  const renderReviewsContent = () => {
    return (
      <>
        {defaultAttributes.card_position == 'top' && renderReviewItems()}
        {renderReviewsInfo()}
        {reviewService === 'woocommerce_native_reviews' && (
          <div className="flex md:flex-row md:w-auto w-full flex-col md:gap-6 gap-3 md:px-8 md:py-3 p-2 border-[1px] border-gray-400">
            <button
              className={`py-2.5 px-8 text-white text-sm font-light ${
                selectedReviewsWithImage ? 'bg-gray-400' : 'bg-black'
              }`}
              disabled={product?.metaData?.wooProductReviews?.stats?.count_with_images === 0}
              onClick={() => setSelectedReviewsWithImage((prev: boolean) => !prev)}
            >
              <Camera strokeColor="#ffffff"></Camera>
              With images ({product?.metaData?.wooProductReviews?.stats?.count_with_images ?? 0})
            </button>
            <button
              className={`py-2.5 px-4 text-white text-sm font-light ${
                selectedVerifiedReviews ? 'bg-gray-400' : 'bg-black'
              }`}
              disabled={product?.metaData?.wooProductReviews?.stats?.verified_count === 0}
              onClick={() => setSelectedVerifiedReviews((prev: boolean) => !prev)}
            >
              <Badge strokeColor="#ffffff"></Badge>
              Verified ({product?.metaData?.wooProductReviews?.stats?.verified_count ?? 0})
            </button>
            <select
              className="bg-black py-2.5 px-8 text-white text-sm font-light text-center"
        ...
        {defaultAttributes.card_position == 'bottom' && renderReviewItems()}
      </>
    );
  };
  ...
  return (
    ...
    >
      ...
      {reviewService === 'woocommerce_native_reviews' && <>{renderReviewsContent()}</>}
    </div>
  );
}
